...
int AccidentsCollator::processArgs(int argc, char *argv[]) {
  ...
  if (cmdLineArgs.modelFilePath.empty() || cmdLineArgs.accidentsFile.empty() ||
      cmdLineArgs.outputFile.empty()) {
...
Timestamp AccidentsCollator::toTimestamp(const std::string &timestamp) const {
  struct tm time;
  ...
  if (chr != eos) {
    ...
    std::cerr << "chr = " << (long)chr << ", eos = " << (long)eos << std::endl;
...
int AccidentsCollator::run(int argc, char *argv[]) {
  ...
  if (!accidentsData.good()) {
    std::cerr << "Unable to read accidents file " << cmdLineArgs.accidentsFile
              << std::endl;
  }
...
double AccidentsCollator::logScaleAcc(const double acc, const double minAcc,
...
void AccidentsCollator::genXFig(const AccidentsMap &accidents,
  ...
  if (!cmdLineArgs.shapeFilePath.empty()) {
    ...
    if (!shpFile.loadShapes(cmdLineArgs.shapeFilePath,
      ...
      std::cerr << "Error loading shape/DBF file: " << cmdLineArgs.shapeFilePath
                << " or " << cmdLineArgs.dbfFilePath << std::endl;
      return;
    }
    shpFile.genXFig(xfig, xClip, yClip, cmdLineArgs.figScale, false, {}, false,
                    cmdLineArgs.xfigFilePath);
  }
  ...
  for (const auto &entry : accidents) {
    ...
    os << nodeID << '\t' << node.latitude << '\t' << node.longitude << '\t'
       << info.count << '\t' << info.injuries << '\t' << info.deaths << '\t'
       << info.info;
...
void AccidentsCollator::write(const AccidentsMap &accidents,
  ...
  for (const auto &entry : accidents) {
    ...
    os << nodeID << '\t' << node.osmId << '\t' << node.latitude << '\t'
       << node.longitude << '\t' << info.count << '\t' << info.injuries << '\t'
...
AccidentsCollator::collate(std::istream &accidentsData, const OSMData &model,
                           std::ostream &out) {
  ...
  const Timestamp endDate = toTimestamp(cmdLineArgs.endDate + " 11:59:59 PM");
  // Detect whether the input file is CSV or TSV based on extension.
  // CSV (e.g., Chicago open data portal) uses different column indices
  // and ISO 8601 timestamps; TSV uses the legacy column layout.
  const bool isCSV = isCsvFile(cmdLineArgs.accidentsFile);
  // Column indices differ between TSV and CSV formats:
  //   TSV: date=3,  lat=46, lon=47, injuries=36, deaths=37
  //   CSV: date=2,  lat=45, lon=46, injuries=35, deaths=36
  const int dateCol = isCSV ? 2 : 3;
  const int latCol = isCSV ? 45 : 46;
  const int lonCol = isCSV ? 46 : 47;
  const int injuriesCol = isCSV ? 35 : 36;
  const int deathsCol = isCSV ? 36 : 37;
  ...
  do {
    const auto batch = getNextBatch(accidentsData, lNum, cmdLineArgs.batchSize);
    if (batch.empty()) {
...
#pragma omp parallel for
    for (size_t i = 0; (i < batch.size()); i++) {
      ...
      const std::string dateStr =
          isCSV ? getCSVColumn(line, dateCol) : getColumn(line, dateCol);
      const Timestamp accTime =
          isCSV ? toTimestampISO(dateStr) : toTimestamp(dateStr);
      ...
      const auto latStr =
          isCSV ? getCSVColumn(line, latCol) : getColumn(line, latCol);
      const auto lonStr =
          isCSV ? getCSVColumn(line, lonCol) : getColumn(line, lonCol);
...
#pragma omp critical(cerr)
      ...
      for (size_t nodeID = 0; (nodeID < model.nodeList.size()); nodeID++) {
...
#pragma omp critical(cerr)
        std::cerr << "Unable to find node for accident on line #" << (lNum - i)
                  << " with lat: " << lat << ", lon: " << lon
                  << ". Nearest node was: " << nearNodeID
                  << ", distance: " << nearNodeDist << " miles.\n";
        continue;
      }
      // Ok, we got a node for this accident. Let's collate the data.
      const std::string injStr = isCSV ? getCSVColumn(line, injuriesCol)
                                       : getColumn(line, injuriesCol, "0");
      const std::string dthStr = isCSV ? getCSVColumn(line, deathsCol)
                                       : getColumn(line, deathsCol, "0");
      const int injuries = injStr.empty() ? 0 : std::stoi(injStr);
      const int deaths = dthStr.empty() ? 0 : std::stoi(dthStr);

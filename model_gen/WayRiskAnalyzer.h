...
/**
 * A map to track the OSM way ID and the number of actual accidents
 * associated with each way.  This map is generated via the
 * assignAccidents method.  This map is used to generate the final
 * list of Ways in each route.
 */
using WayIDAccidentsMap = std::unordered_map<long, int>;

/**
 * A map to track the NodeID (and not the OSM-ID) of a node and the number of
 * accidents that occured at that node.  This map is loaded with data
 * output by by the accident collator (e.g., accident_nodes_2022.tsv).
 * The data file is specified via the \c --accident-nodes command-line
 * argument.
 */
using NodeAccidentsMap = std::unordered_map<long, int>;
    ...
    */
    ...
    public:
        ...
        */
        ...
        /**
         * This map is loaded with data output by by the accident
         * collator (e.g., accident_nodes_2022.tsv).  The data file is
         * specified via the \c --accident-nodes command-line
         * argument.
         */
...
private: 
   ...
   /**
    ...
    /**
     * Internal helper method to load the accidents data from a given
     * TSV file that was generated by the accident collator program.
     *
     * \param[in] accidentNodesTSV Path to the TSV file output by the
     * accident_collator.  This file is assumed to be in a specific
     * column order of:
     *
     * \begincode
     * # NodeID	OSMId	Latitude	Longitude	AccCount	Injuries	Deaths	OtherInfo
     * \endcode
     *
     * \return A map with OSM id and the number of accidents that have
     * been assigned to each node.
     */
    NodeAccidentsMap loadAccidentData(const std::string& accidentNodesTSV) const;

    WayIDAccidentsMap assignAccidents(const NodeAccidentsMap& nodesAccidents,
...
private:
    ...
    /**
     * The number of accidents that have been reported on a given way.
     * This map is populated by the assignAccidents method.
     */

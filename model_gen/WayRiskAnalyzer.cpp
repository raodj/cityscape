...
Timestamp WayRiskAnalyzer::toTimestamp(const std::string &timestamp) const {
  struct tm time;
...
WayRiskAnalyzer::getBatch(std::istream &taxiData, const Timestamp &startDay,
  ...
  const bool isCSV = isCsvFile(cmdLineArgs.taxiRidesFile);
  for (std::string line;
       (batch.size() < batchSize) && std::getline(taxiData, line);) {
    // If the current entry is not within our time window, ignore it.
    const std::string startTime =
        isCSV ? getCSVColumn(line, 2) : getColumn(line, 2);
    const std::string endTime =
        isCSV ? getCSVColumn(line, 2) : getColumn(line, 2);
    const std::string stAreaID =
        isCSV ? getCSVColumn(line, stAreaCol) : getColumn(line, stAreaCol);
    const std::string endAreaID =
        isCSV ? getCSVColumn(line, endAreaCol) : getColumn(line, endAreaCol);
    ...
    const Timestamp startTS =
        isCSV ? toTimestampISO(startTime) : toTimestamp(startTime);
    const Timestamp endTS =
        isCSV ? toTimestampISO(endTime) : toTimestamp(endTime);
    if (isHigher(startDay, startTS) && isHigher(endTS, endDay) &&
      ...
      const std::string distStr =
          isCSV ? getCSVColumn(line, 5) : getColumn(line, 5);
      if (distStr.empty()) {
...
void WayRiskAnalyzer::processTaxiData(std::istream &taxiData,
...
#pragma omp critical(processTaxiData_ways)
    ...
    std::cout << "Completed batch processing in  " << elapsedTime.count() / 1e3
              << " seconds. Total paths: " << pathCount << std::endl;
...
void WayRiskAnalyzer::updateNodes(const Path &path, NodeVisitMap &nodeVisits,
                                  WayVisitMap &wayVisits, const int startMinute,
                                  const int maxEntries) {
...
std::string WayRiskAnalyzer::minToStr(const int min) const {
  const int hours = min / 60, mins = min % 60;
  return std::to_string(hours) + ":" + std::to_string(mins);
}

void WayRiskAnalyzer::writeNodeSummary(std::ostream &os) const {
...
void WayRiskAnalyzer::writeWaysSummary(std::ostream &os) const {
  // First print the fixed part of the header for the file.
  os << "# Ways Summary\n"
     << "# Generated using command-line: " << cmdLineArgs.fullCmdLine
...
WayRiskAnalyzer::loadAccidentData(const std::string &accidentNodesTSV) const {
  ...
  for (std::string line; std::getline(tsv, line);) {
    ...
    std::istringstream(line) >> nodeID >> osmID >> dummy >> dummy >> accCount;

...
void ShpBldCatalog::buildCatalog(const std::string &shpFilePath,
                                 const std::string &dbfFile,
                                 const OSMData &model) {
  if (!shpFile.loadShapes(shpFilePath, dbfFile)) {
    std::cerr << "Error loading shape/DBF file: " << shpFilePath << " or "
              << dbfFile << std::endl;
    return;
  }
...
#pragma omp parallel reduction(+ : assignedCount)
  {
    ...
    while (bldIter != bldEnd) {
      ...
      if (assignToRing != -1) {
        ...
        // Try field names used by different census tract vintages:
        std::string ringID = ring.getInfo("geoid10");
        if (ringID.empty()) {
          ringID = ring.getInfo("GEOID");
        }
        if (ringID.empty()) {
          ringID = ring.getInfo("GEOID20"); // id name for 2020 census tract
        }
        ASSERT(!ringID.empty());

        // if (ringID.empty()) {
        //   // Shape has no recognisable ID field — skip this building.
        //   bldIter++;
        //   continue;
        // }

...
void getXYValues(double latitude, double longitude, int &x, int &y,
                 const int size, const bool wrap) {
...
void getLatLon(const int x, const int y, const int MapSize, double &lat,
               double &lon) {
  lat = lon = 0;
...
bool inBetween(const double val1, const double val2, const double val3) {
...
void getPoint(const double node1Lat, const double node1Lon,
              const double node2Lat, const double node2Lon, const double dist,
              double &pointLat, double &pointLon, const bool offset) {
...
std::string getCSVColumn(const std::string &line, const int column) {
  ...
  while (pos <= line.size()) {
    ...
    size_t fieldStart = pos + (quoted ? 1 : 0);
    size_t fieldEnd;
    if (quoted) {
    ...
    if (currCol == column) {
      return line.substr(fieldStart, fieldEnd - fieldStart);
    }
    // Advance past this field
    pos = fieldEnd;
    if (quoted && pos < line.size()) {
      pos++; // skip closing '"'
    }

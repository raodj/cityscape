...
std::string DrawShapes::getColumn(const std::string &line, const int column,
                                  const char delim) {
...
void DrawShapes::addAnnotations(const std::string &annotPath,
  ...
  for (std::string line; std::getline(annotTSV, line);) {
    ...
    if (areaID.empty()) {
      std::cerr << "Unable to extract areaID from column " << areaColIdx
    ...
    const int ringIdx = shpFile.findRing(areaID, {"area_numbe", "geoid10"});
    if (ringIdx == -1) {
...
void DrawShapes::generateFig() {
  ...
  shpFile.genXFig(fig, xClip, yClip, cmdLineArgs.figScale, true,
                  cmdLineArgs.labelColNames, true, cmdLineArgs.xfigFilePath);
  ...
  if (!cmdLineArgs.wayRiskFile.empty()) {
    ...
    if (!waysRiskTSV) {
      std::cerr << "Error reading TSV file " << cmdLineArgs.wayRiskFile
                << std::endl;
      return;
    }
...
int DrawShapes::loadShapeFile() {
  // Load data from the shapefile and dbf file.
  if (!shpFile.loadShapes(cmdLineArgs.shapeFilePath, cmdLineArgs.dbfFilePath)) {
...
int DrawShapes::processArgs(int argc, char *argv[]) {
  ...
  ArgParser::ArgRecord arg_list[] = {
      ...
      {"--xfig", "The output XFig file", &cmdLineArgs.xfigFilePath,
       ArgParser::STRING},
      {"--scale", "The size of the output map", &cmdLineArgs.figScale,
       ArgParser::INTEGER},
...
int DrawShapes::getColor(const int value) const {
  if (value < 10) {
    return 32;
  }
...
void DrawShapes::plotRiskWays(std::istream &waysRiskTSV, XFigHelper &xfig,
  ...
  for (std::string line; std::getline(waysRiskTSV, line);) {
    ...
    const std::string wayInfo =
        "OSM_WayID: " + words[0] + ", Name: " + words[1] +
        ", kind: " + words[2] + ", speed: " + words[3] +
    ...
    for (int i = 0; (i < nodeCount); i++) {
      ...
      int xfigX, xfigY;
      getXYValues(lat, lon, xfigX, xfigY, xfigSize * XFIG_SCALE, false);
      xfigX -= xClip;
      xfigY -= yClip;
  ...
  const std::vector<double> tics = {0,    1,  1,     100, 50,
                                    5000, 75, 32500, 100, 100000};

    ...
    for (int argument = 1; argument < argc; argument++) {
        ...
        for (int match = 0; ((match < NumArgs) && (argument < argc)); match++){
            ...
            if ((argRec.command != argv[argument]) &&
                (argRec.shortCommand != argv[argument])) {
            ...
            if ((argRec.type != BOOLEAN) && (argRec.type != INV_BOOLEAN) &&
                (argRec.type != STRING_LIST)) {
                if ((argument + 1 == argc) ||
                    ((argv[argument + 1][0] == '-') &&
                     (argv[argument + 1][1] == '-'))) {
            ...
            case INV_BOOLEAN:
                removeArgument(argument, argc, argv);
                *(reinterpret_cast<bool*>(argRec.data)) = false;
                break;
            case INTEGER:
                removeArgument(argument, argc, argv);
...
ArgParser::checkRemainingArguments(int argc, char* argv[],
    ...
    const std::string HelpStr("--help"), shortHelpStr("-h");
    for (int i = 1; (i < argc); i++) {
        if ((HelpStr == argv[i]) || (shortHelpStr == argv[i])) {
...
std::ostream& operator<<(std::ostream& os, ArgParser& ap) {
    ...
    static const ArgParser::ArgRecord HelpArgRec =
        {"--help", "-h", "Print this message", &helpFlag, ArgParser::BOOLEAN};
    ...
    for (size_t i = 0; (i < ap.argRecords.size()); i++) {
        maxLen = std::max(maxLen, (int) (ap.argRecords[i].command.size() +
                                         ap.argRecords[i].shortCommand.size()));
    }
    ...
    for (size_t i = 0; (i < ap.argRecords.size()); i++) {
        ...
        const int argLen = argRec.command.size() + argRec.shortCommand.size();
        if (argRec.type != ArgParser::MAIN_MESSAGE) {
            ap.printArg(os, IndentStr, maxLen - argLen + numSpaces, argRec);
        } else {
            os << argRec.help << std::endl;
        }
    }
    // Print the omnipresent help option
    ap.printArg(os, IndentStr, maxLen + numSpaces - 8, HelpArgRec);
    return os;
}
...
ArgParser::getValue(const ArgRecord& argRec) {
    ...
    case BOOLEAN:
        ...
        break;
    case INV_BOOLEAN:
        retVal << (*(reinterpret_cast<bool*>(argRec.data)) ? "false" : "true");
        break;        
    case INTEGER:
...
ArgParser::printArg(std::ostream& os, const std::string& indentStr,
    ...
    os << argRec.command << " (" << argRec.shortCommand << ")";
    ...
    const size_t spaceRemaining = maxWidth - indentStr.size() -
        argRec.command.size() - numSpaces - 5;
...
bool
ArgParser::checkArg(const bool condition, const void* data,
                    const std::string& message, std::ostream& os) const {
    if (!condition) {
        // First print the user-specified message
        os << message << ". Use the following option:\n";
        // Find the arg-parser entry corresponding to the specified data.
        for (const ArgRecord& argRec : argRecords) {
            if (argRec.data == data) {
                printArg(os, "  ", 2, argRec);
            }
        }
    }
    return condition;  // Return supplied value
}
